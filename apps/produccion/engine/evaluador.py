    ...
    def evaluar(nodo):
        ...
        if isinstance(nodo, ast.Name):
            if nodo.id not in variables:
                raise ValueError(
                    f"Variable no definida: {nodo.id}"
                )
            ...
            if valor is None:
                raise ValueError(
                    f"La variable {nodo.id} no tiene valor."
                )
        ...
        raise ValueError(
            f"Expresión no permitida: {type(nodo).__name__}"
        )
...
def evaluar_metrica(metrica, objeto):
    if not metrica.activa:
        raise ValueError(
            f"La métrica {metrica.codigo} está inactiva."
        )

    if not metrica.formula:
        raise ValueError(
            f"La métrica {metrica.codigo} no tiene fórmula."
        )
    ...
    for variable in (
        ...
        variables[variable.codigo] = resolver_variable(
            variable,
            objeto,
        )
