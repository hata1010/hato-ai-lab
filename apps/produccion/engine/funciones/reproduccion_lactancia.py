...
class IntervaloEntrePartos(FuncionBase):
    ...
    def ejecutar(self, contexto):
        ...
        eventos = list(animal.eventos_reproductivos.filter(tipo_evento="parto").order_by("-fecha")[:2])
        if len(eventos) < 2:
            raise ErrorDatosInsuficientes("Se requieren al menos 2 partos válidos para calcular el intervalo entre partos.")
...
class DiasAbiertos(FuncionBase):
    ...
    def ejecutar(self, contexto):
        ...
        ultimo_parto = animal.eventos_reproductivos.filter(tipo_evento="parto").order_by("-fecha").first()
        if ultimo_parto is None:
            raise ErrorDatosInsuficientes("Se requiere un parto registrado para calcular días abiertos.")
        concepcion = animal.eventos_reproductivos.filter(
            tipo_evento="diagnostico_gestacion",
            resultado_gestacion="prenada",
            fecha__gt=ultimo_parto.fecha,
        ).order_by("fecha").first()
        if concepcion is None:
            raise ErrorDatosInsuficientes("No existe un diagnóstico de gestación positivo posterior al último parto.")
...
class ProduccionAcumuladaLactancia(FuncionBase):
    ...
    def ejecutar(self, contexto):
        ...
        if not controles:
            raise ErrorDatosInsuficientes("Se requiere al menos un control de leche para calcular producción acumulada.")
        unidades = {control.unidad for control in controles}
        if len(unidades) != 1:
            raise ErrorDatosInsuficientes("No se puede acumular una serie de leche con unidades mezcladas sin una conversión explícita.")
...
class DuracionLactancia(FuncionBase):
    ...
    def ejecutar(self, contexto):
        ...
        if lactancia.fecha_secado is None:
            raise ErrorDatosInsuficientes("La lactancia aún no tiene fecha de secado; la duración final no está determinada.")
