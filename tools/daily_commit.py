...
never commits unrelated working-tree changes. It can optionally push the
result to the configured upstream branch.
...
def main() -> int:
    ...
    parser.add_argument("--dry-run", action="store_true", help="show what would be staged without committing")
    ...
    # Stage only the continuity subsystem. This protects application code and
    # unrelated user changes from being swept into an automatic memory commit.
